...
['Seed Cloud','Suspended capsules','A suspended field of seed forms carrying the knowledge, diversity and potential held inside every variety.','https://ik.imagekit.io/18ab23oqaj/zone2.png'],
['The Rice Archive','Living archive','More than 200 varieties preserve colour, form, origin and the generations of hands behind them.','https://ik.imagekit.io/18ab23oqaj/zone3.png'],
['Rice Through Time','Timeline corridor','A walk through ten thousand years of cultivation, exchange, innovation and human memory.','https://ik.imagekit.io/18ab23oqaj/zone4.png'],
['How the World Eats Rice','Forty metre table','One continuous table reveals how rice changes across countries while staying a shared human language.','https://ik.imagekit.io/18ab23oqaj/zone5.png'],
...
['Rice Mirror','Responsive installation','A responsive installation turns every visitor into part of the field.','https://ik.imagekit.io/18ab23oqaj/zone8.png'],
...
zones.forEach(function(z,i){
  stage+='<img class="gf-stage-image'+(i===0?' on':'')+'" src="'+z[3]+'" alt="'+z[0]+', Experience Zone '+(i+1)+'" data-stage="'+i+'"'+(i?' loading="lazy"':'')+' decoding="async">';
...
function shape(num){var off=document.createElement('canvas'),ow=Math.max(420,Math.floor(w*.44)),oh=Math.max(400,Math.floor(h*.6));off.width=ow;off.height=oh;var o=off.getContext('2d');o.fillStyle='#fff';o.textAlign='center';o.textBaseline='middle';o.font='700 '+Math.floor(oh*.82)+'px Poppins, sans-serif';o.fillText(String(num+1).padStart(2,'0'),ow/2,oh*.52);var data=o.getImageData(0,0,ow,oh).data,samples=[];for(var y=0;y<oh;y+=4)for(var x=0;x<ow;x+=4)if(data[(y*ow+x)*4+3]>90)samples.push([x+w*.52,y+(h-oh)/2-14]);for(var i=samples.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),q=samples[i];samples[i]=samples[j];samples[j]=q}target=samples}
...
var field=main.querySelector('.gf-field');
field.addEventListener('pointermove',function(e){mouse.x=e.clientX;mouse.y=e.clientY});
field.addEventListener('pointerleave',function(){mouse.x=-9999;mouse.y=-9999});
addEventListener('resize',resize);
if(document.fonts&&document.fonts.ready)document.fonts.ready.then(function(){resize();frame()});else{resize();frame()}
})();
