...
var tabs='';
zones.forEach(function(z,i){tabs+='<button class="gf-pick" type="button" role="tab" aria-selected="'+(i===0?'true':'false')+'" data-zone="'+i+'"><b>'+String(i+1).padStart(2,'0')+'</b><span>'+z[0]+'</span></button>'});
...
var picker=main.querySelector('.gf-picker'),copy=main.querySelector('.gf-copy'),buttons=[].slice.call(main.querySelectorAll('.gf-pick')),field=main.querySelector('.gf-field');
zones.forEach(function(z){var pre=new Image();pre.src=z[3]});
var canvas=main.querySelector('.gf-canvas'),ctx=canvas.getContext('2d'),dpr=1,w=0,h=0,pts=[],target=[],active=0,mouse={x:-9999,y:-9999},gold='#ebb341',muted='#6f675a';
...
function seed(){var n=Math.min(2100,Math.max(1100,Math.floor(w*h/700)));for(var i=0;i<n;i++)pts.push({x:Math.random()*w,y:Math.random()*h,vx:0,vy:0,a:.3+Math.random()*.6,s:1.1+Math.random()*2.1,r:Math.random()*6.28,g:Math.random()<.26})}
function shape(num){var oh=Math.max(300,Math.floor(h*.42)),ow=Math.floor(oh*1.25),off=document.createElement('canvas');off.width=ow;off.height=oh;var o=off.getContext('2d');o.fillStyle='#fff';o.textAlign='center';o.textBaseline='middle';o.font='700 '+Math.floor(oh*.98)+'px Poppins, sans-serif';o.fillText(String(num+1).padStart(2,'0'),ow/2,oh*.52);var data=o.getImageData(0,0,ow,oh).data,samples=[],ox=w*.62-ow/2,oy=h*.47-oh/2;for(var y=0;y<oh;y+=3)for(var x=0;x<ow;x+=3)if(data[(y*ow+x)*4+3]>90)samples.push([x+ox,y+oy]);for(var i=samples.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),q=samples[i];samples[i]=samples[j];samples[j]=q}target=samples}
function select(i){if(i===active)return;active=i;buttons.forEach(function(b,k){b.setAttribute('aria-selected',k===i?'true':'false')});copy.classList.add('swap');setTimeout(function(){copy.innerHTML='<small class="gf-count">'+zones[i][1]+'</small><h1>'+zones[i][0]+'</h1><p>'+zones[i][2]+'</p>';copy.classList.remove('swap')},140);shape(i)}
